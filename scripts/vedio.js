...
const displayVideos = (videos) => {
    ...
    <img
      ...
      alt="Shoes" />
      ${video.others.posted_date?.length == 0 ? "" : `<span class="absolute text-xs bottom-2 right-2 rounded bg-black p-1 text-white">${getTimeString(video.others.posted_date)}</span>`
            }
...
const DisplayCategories = (categories) => {
    ...
    categories.forEach(item => {
        ...
        `<button onclick ="videoCategories($
        {item.category_id})" class="btn">
        ${item.category}
        </button>
        `;
